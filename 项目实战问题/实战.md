...
# 十五、IDEA内部继承热部署插件 jrebel

我先给出自己的步骤，也可以参考别人的博客https://blog.csdn.net/weixin_42694593/article/details/126448828

插件市场首先下载JRebel

之后需要激活插件。

![image-20240414202453269](images/image-20240414202453269.png)

![image-20240414202505527](images/image-20240414202505527.png)

![image-20240414202512752](images/image-20240414202512752.png)

![image-20240414202523830](images/image-20240414202523830.png)





后面就可以启动项目了，但是如果项目没办法正常启动，

![image-20240414202552413](images/image-20240414202552413.png)
...
这种代理本身其实是一种编译时增强，这个在你自己编译成字节码的时候就会重写class文件了，直接在字节码层面修改了代码。

需要引入一个插件，这个就是告诉maven编译的时候使用aspectj的编译器
...
我们这个实在编译时修改的字节码，其实有一种代理可以变成在**类加载**的时候完成类的代理，但是注意需要启动时加入一个参数

```
-javaagent:D:/maven/apache-maven-3.9.6/repo/org/aspectj/aspectjweaver/1.9.7/aspectjweaver-1.9.7.jar
```

后面这个是什么呢，这个是自己的aspectjweaver-1.9.7.jar包在maven的存储地址

那么对于spring自己的aop增强呢？其实**Spring AOP 属于运行时增强，而 AspectJ 是编译时增强。** Spring AOP 基于代理(Proxying)，而 AspectJ 基于字节码操作(Bytecode Manipulation)。
...
有两种方式。代理方式本身是一种在运行时生成字节码进行加载的方式。

对于这一部分的原理，可以查看javaweb中Spring高级部分的知识来了解他们的底层实现原理。
...
//            Object result = method.invoke(target, args); //用反射来进行调用
//            Object result = methodProxy.invoke(target, args);//内部没用反射，需要目标，target就是目标，也就是被代理的对象（spring也是使用这种方式）
            Object result = methodProxy.invokeSuper(p, args); //内部没用反射，需要代理，p就是代理
