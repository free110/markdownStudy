...
当调用Component1的register方法之后，Component2就会接受到信息。不过需要注意需要添加注解@EventListener
...
可以通过getEnvironment的方法得到系统或者这个项目的properties的配置文件中的内容。
...
返回的就是我们索要寻找的资源，需要注意的是，所有的配置文件都可以称为资源
...
public class TestBeanFactory {
    ...
    public static void main(String[] args) {
        ...
        //此时还是无法加载解析注解的，需要后处理器才能解析
        beanFactory.registerBeanDefinition("config", beanDefinition);
        // 给 BeanFactory 添加一些常用的后处理器
            //此时还是无法加载解析注解的，需要后处理器执行起来才能解析
        AnnotationConfigUtils.registerAnnotationConfigProcessors(beanFactory);
        // BeanFactory 后处理器主要功能，补充了一些 bean 定义
        beanFactory.getBeansOfType(BeanFactoryPostProcessor.class).values().forEach(beanFactoryPostProcessor -> {
            //开始执行起来后处理器，这次才能正确的解析注解（例如: @Configuration）。但是有一些注解（例如:@Autowired）是无法解析的，因为有些bean还并没有真正的实例化，仅仅只是定义了而已。
...
//        System.out.println("第1次" +  beanFactory.getBean(Bean1.class).getBean2());
        ...
        //注意不要和第一次同时执行，因为是懒加载的，只会创建一次。
        System.out.println("第2次" + beanFactory.getBean(Bean1.class).getBean2());
...
//
//        beanFactory.preInstantiateSingletons(); // 准备好所有单例，这个就是在bean真正被调用懒加载之前将所有的单例对象实例化起来
//        System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ");
////        System.out.println(beanFactory.getBean(Bean1.class).getBean2());
        //因为前面bean的处理器顺序改变，那么注解优先级改变，这里也就变成了Bean4
...
注意：DefaultListableBeanFactory太过接近底层，就没有自动的设置，所以很多东西都需要我们手动调用。这个和ApplicationContext 不一样，他就帮我们做了很多设置，不需要我们手动调用太多的api
...
3. 会用 arthas 的 jad 工具反编译第 17 次调用生成的代理类。（这里黑马的理解也许并不正确，其实本身并没有新生成代理类，下面是我问gpt给出的回答：

   首先，我们需要明确一点，Java的动态代理是在运行时生成一个实现了一组接口的新类，并将接口的方法调用转发给一个调用处理程序（InvocationHandler）。这个处理程序可以做各种各样的操作，包括调用目标对象的相应方法。

   在经过一定次数的调用后，JVM会对反复调用的方法进行优化，这个优化是在字节码层面进行的。具体来说，JVM会生成一段新的字节码，这段字节码相当于直接调用了目标对象的方法，绕过了反射。

   实际上，JVM在生成这段字节码的时候会带上一些额外的信息，这里面包括目标方法的指针等信息。这样在运行这段字节码的时候，能快速找到并执行目标方法，大大提高了执行效率。

   所以，这个"生成的代理对象"并不是一个新的Java对象，而是一段新的字节码。这也是为什么说这个优化发生在JIT编译阶段，是透明的，我们并不能直观地看到这个过程。）

   注意：增强后的代理类会实例化，原始的类也会实例化，增强代理类需要持有原始的类。我当时有疑问，为什么持有了原始类，不直接调用原始方法而是使用反射来执行呢，我自己感觉是因为jdk动态代理调用了InvocationHandler，这个回调函数InvocationHandler可能对原始对象的很多方法进行增强，并且在代理里面重写的那些方法都回调了InvocationHandler，InvocationHandler这个是抽取出来的代码，为了通用性使用了反射来执行原始类的方法，所以使用的反射（这些是我的理解，不一定对）。这是第二次理解，发现他们可以选择实现用原始类执行方法，只不过没有选择而已；我看到下面的cglib就是可以使用原始类进行方法调用，看来jdk代理只是没有选择使用而已。
...
4. ProxyFactory 用来创建代理
   ...
   * 如果没有指定接口，或者 proxyTargetClass = true（这种为true一定使用cglib的代理实现），使用 ObjenesisCglibAopProxy
...
![image-20240428194445858](images/image-20240428194445858.png)
